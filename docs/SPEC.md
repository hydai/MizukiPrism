...
MizukiPrism 讓浠Mizuki 的粉絲能快速瀏覽、搜尋並播放她在歌回直播中演唱過的所有歌曲，透過嵌入 YouTube VOD 的方式提供類似 Spotify 的音樂探索體驗。

### 1.2 Users

| 角色 | 描述 | 核心任務 |
|------|------|----------|
| 粉絲（Fan） | 浠Mizuki 的觀眾，想回味或發現她唱過的歌 | 瀏覽歌曲目錄、搜尋特定歌曲、播放特定版本、建立與管理播放清單 |
...
| 影響指標 | 目標 |
|----------|------|
| 歌曲發現速度 | 粉絲能在 30 秒內找到浠Mizuki 唱過的任何一首歌 |
...
| 功能 | 描述 | 使用者 |
|------|------|--------|
| 歌曲目錄（Song Catalog） | 列出所有浠Mizuki 演唱過的歌曲，含歌名、原唱者、分類標籤 | 粉絲 |
...
| 直播主頁面（Streamer Profile） | 頁面頂部顯示浠Mizuki 的頭像、簡介、社群連結、歌曲總數 | 粉絲 |
| YouTube 直連（Open in YouTube） | 每個演出版本提供直接連結，可在 YouTube 上以正確時間戳開啟 | 粉絲 |
| 外部追蹤按鈕（External Follow） | 點擊後在新分頁開啟浠Mizuki 的 YouTube 頻道以進行訂閱（非站內追蹤） | 粉絲 |
...
| 階段 | 內容 |
|------|------|
| 情境（Context） | 粉絲想知道浠Mizuki 有沒有唱過某首歌 |
...
| 階段 | 內容 |
|------|------|
| 情境（Context） | 浠Mizuki 完成了一場歌回直播，策展人需要將新演唱的歌曲資料加入系統 |
...
| 階段 | 內容 |
|------|------|
| 情境（Context） | 粉絲想看浠Mizuki 最近唱了什麼歌 |
...
| 任意 | 點擊 YouTube 連結 | 在新分頁開啟浠Mizuki 的 YouTube 頻道 |
| 任意 | 點擊追蹤按鈕 | 在新分頁開啟浠Mizuki 的 YouTube 頻道訂閱頁面 |
...
> **設計參照**：視覺設計的權威來源為 Pencil 設計檔 `~/Library/CloudStorage/Dropbox/PencilDesign/MizukiPrism`。桌面版為畫框 "Spotify - 浠Mizuki Music"（1440×900），行動裝置版為畫框 "Spotify Mobile - 浠Mizuki"（390×844）。以下各小節描述該設計檔中的佈局決策與元件結構。
...
| 版本 | Version | 浠Mizuki 在特定直播場次中演唱某首歌曲的一次演出。版本屬於且僅屬於一首歌曲和一個直播場次。每個版本包含開始時間戳、選填的結束時間戳與選填的演出備註。 |
