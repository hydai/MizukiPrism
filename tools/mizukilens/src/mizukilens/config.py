"""Channel configuration management for MizukiLens.

Handles reading, writing, and interactively editing the TOML configuration
file at ~/.config/mizukilens/config.toml.
"""
...
CONFIG_DIR = Path.home() / ".config" / "mizukilens"
...
def run_config_command() -> None:
    """Entry point for `mizukilens config`."""
    from rich.console import Console
    ...
    if cfg is None:
        ...
        console.print()
        console.print("[bold cyan]MizukiLens — 初始設定[/bold cyan]")
...
def _display_config(console: Any, cfg: dict[str, Any]) -> None:
    ...
    console.print()
    console.print(f"[bold cyan]MizukiLens 設定[/bold cyan]  （設定檔：{CONFIG_PATH}）")
