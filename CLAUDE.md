...
```
app/
├── components/        React components (AlbumArt, MiniPlayer, NowPlayingModal, QueuePanel, SyncedLyrics, ...)
├── contexts/          React contexts (PlayerContext, PlaylistContext, FanAuthContext)
├── api/               Next.js API routes (songs, streams, metadata, lyrics, playlists, auth)
├── page.tsx           Main song catalog page (~2000 lines)
lib/
...
```bash
cd tools/mizukilens
.venv/bin/python3 -m pytest tests/ -v   # Run Python tests (740+)
